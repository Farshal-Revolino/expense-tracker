...
export default function Register({ onRegister, onSwitchToLogin }) {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => { setLoading(false); onRegister(userData); }, 500);
  };
    ...
    <div className="min-h-screen mesh-bg flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Start managing your finances</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="glass rounded-2xl p-6 space-y-4">
          <div>
            ...
            <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] text-sm text-slate-900 dark:text-white outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all" placeholder="Your name" />
          </div>
          <div>
        ...
        <p className="text-center text-sm text-slate-500 dark:text-slate-400 mt-6">
          ...
          <button onClick={onSwitchToLogin} className="font-bold text-emerald-500 dark:text-emerald-400 hover:underline">Sign in</button>
        </p>
      </div>
    </div>
  );
